...
class ExecutorModelo:
    ...
    def executar_acao(self, symbol, action, info):
        ...
        # Define a alavancagem para o símbolo (padrão 10x)
        # Pode ser configurado no arquivo de configuração risk_config.yaml
        alavancagem = self.risk_config['risk_manager'].get('leverage', 10)
        
        # Tenta aplicar a alavancagem ao símbolo
        try:
            self.bot.set_leverage(symbol, alavancagem)
        except Exception as e:
            logger.warning(f"Não foi possível configurar alavancagem {alavancagem}x para {symbol}: {e}")
        
        # Verifica alavancagem atual
        leverage_atual = self.bot.get_leverage(symbol) or alavancagem
        logger.info(f"Operando {symbol} com alavancagem {leverage_atual}x")
        
        # Ajusta os cálculos de stop loss e take profit considerando a alavancagem
        # Com alavancagem maior, precisa de mais espaço para respirar (menor percentual)
        # Distância do SL ajustada para alavancagem
        sl_distance_pct = min(2.0 / leverage_atual, 0.5)  # Máx 2% / alavancagem, mínimo 0.5%
        
        # Calcula níveis de stop loss e take profit
        # Para LONG (compra)
        if action == 1:
            # Stop loss ajustado para alavancagem
            stop_loss = preco_atual * (1 - sl_distance_pct)
            # Take profit com base no risk/reward ratio (3:1 por padrão)
            rr_ratio = self.risk_config['risk_manager'].get('rr_ratio', 3.0)
            take_profit = preco_atual + (preco_atual - stop_loss) * rr_ratio
        # Para SHORT (venda)
        elif action == 2:
            # Stop loss ajustado para alavancagem
            stop_loss = preco_atual * (1 + sl_distance_pct)
            # Take profit com base no risk/reward ratio (3:1 por padrão)
            rr_ratio = self.risk_config['risk_manager'].get('rr_ratio', 3.0)
            take_profit = preco_atual - (stop_loss - preco_atual) * rr_ratio
        else:
            stop_loss = None
            take_profit = None
        ...
        if stop_loss and take_profit:
            logger.info(f"Stop Loss: {stop_loss:.4f} | Take Profit: {take_profit:.4f} | Alavancagem: {leverage_atual}x | Risk/Reward: {self.risk_config['risk_manager'].get('rr_ratio', 3.0)}")
        ...
        try:
            ...
            if action == 1 and posicao_atual <= 0:  # Comprar
                ...
                resultado = self.bot.send_order_with_sl_tp(
                    symbol, 
                    'buy', 
                    quantidade, 
                    stop_loss=stop_loss, 
                    take_profit=take_profit,
                    leverage=leverage_atual
                )
